    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <nav class="flex items-center space-x-2 text-sm text-slate-500 dark:text-slate-400">
                ...
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" /></svg>
                <span class="text-amber-600 dark:text-amber-400 font-semibold">Tambah Jadwal</span>
            </nav>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700/50 overflow-hidden">
                <!-- Card Header -->
                <div class="bg-gradient-to-r from-amber-500 to-orange-600 p-6 flex items-center space-x-4">
                    ...
                    <div>
                        <h3 class="text-xl font-bold text-white">Tambah Jadwal Praktik Baru</h3>
                        <p class="text-amber-100/80 text-sm">Lengkapi detail jadwal bertugas dokter di bawah ini</p>
                    </div>
                </div>

                <!-- Form Body -->
                <form action="{{ route('doctor-schedules.store') }}" method="POST" class="p-6 md:p-8">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="day" id="day" required class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 dark:focus:border-amber-400 transition">
                                ...
                                @foreach(['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu'] as $dayName)
                                    <option value="{{ $dayName }}" {{ old('day') == $dayName ? 'selected' : '' }}>{{ $dayName }}</option>
                                @endforeach
                            </select>
                            @error('day')
                                <p class="mt-1 text-xs text-rose-500">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <input type="time" name="end_time" id="end_time" value="{{ old('end_time') }}" required class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 dark:focus:border-amber-400 transition" />
                            @error('end_time')
                                <p class="mt-1 text-xs text-rose-500">{{ $message }}</p>
                            @enderror
                        </div>
