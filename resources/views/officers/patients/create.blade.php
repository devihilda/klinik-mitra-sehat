<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-2 text-sm text-slate-500 dark:text-slate-400">
            ...
            <span>/</span>
            <span class="font-semibold text-slate-800 dark:text-slate-100">Tambah Baru</span>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700/50 overflow-hidden">
                <!-- Card Header -->
                <div class="p-6 border-b border-slate-100 dark:border-slate-700/50 bg-gradient-to-r from-blue-600 to-indigo-600">
                    <h3 class="text-xl font-bold text-white">Registrasi Pasien Baru</h3>
                    <p class="text-blue-100/80 text-sm mt-1">Lengkapi seluruh data di bawah ini untuk mendaftarkan pasien baru ke sistem klinik.</p>
                </div>

                <!-- Form -->
                <form method="POST" action="{{ route('patients.store') }}" class="p-6 md:p-8 space-y-8">
                    @csrf
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pl-11">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name')" placeholder="Masukkan nama lengkap pasien" required />
                                <x-input-error :messages="$errors->get('name')" class="mt-2" />
                            </div>
                            <!-- Email -->
                            <div>
                                <x-input-label for="email" value="Alamat Email" />
                                <x-text-input id="email" name="email" type="email" class="mt-1 block w-full" :value="old('email')" placeholder="contoh@email.com" required />
                                <x-input-error :messages="$errors->get('email')" class="mt-2" />
                            </div>
                            <!-- Password -->
                            <div>
                                {{-- Kebutuhan Praktikum: Password disimpan sebagai plaintext tanpa konfirmasi --}}
                                <x-input-label for="password" value="Password" />
                                <x-text-input id="password" name="password" type="text" class="mt-1 block w-full" placeholder="Masukkan password" required />
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pl-11">
                            ...
                            <div>
                                ...
                                <x-text-input id="phone" name="phone" type="text" class="mt-1 block w-full" :value="old('phone')" placeholder="08xxxxxxxxxx" required />
                    ...
                    <div class="flex items-center justify-end space-x-3 pt-4 border-t border-slate-100 dark:border-slate-700/50">
                        ...
                        <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold rounded-xl shadow-md hover:from-blue-700 hover:to-indigo-700 transition duration-150 ease-in-out transform hover:-translate-y-0.5">
                            Simpan Pasien
                        </button>
                    </div>
                </form>
